...
SPARK_VERSION_NAME=spark-1.3.0-SNAPSHOT-bin
...
for x in {1..16}; 
#do echo "$NOHUP sudo scp install_joao1.3.0.sh f$x:/usr/local/spark/install_joao1.3.0.sh "
do nohup sudo scp install_joao1.3.0.sh f$x:/usr/local/spark/install_joao1.3.0.sh&
done
sleep 3

ssh joao@fbox "dsh -g joao-cluster -c -- sudo /usr/local/spark/install_joao1.3.0.sh $SPARK_VERSION_NAME-$1.tgz"
