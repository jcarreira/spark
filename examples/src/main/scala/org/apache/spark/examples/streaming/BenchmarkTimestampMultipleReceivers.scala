 ...
 *
 * Usage: BenchmarkTimestampMultipleReceivers <numStreams> <host> <port> <batchMillis> [blockInterval]
 ...
 */
object BenchmarkTimestampMultipleReceivers {
  def main(args: Array[String]) {
    if (args.length != 4 && args.length != 5) {
      System.err.println("Usage: BenchmarkTimestampMultipleReceivers <numStreams> <host> <port> <batchMillis> [blockInterval]")
      System.exit(1)
    }
    ...
    sparkConf.setAppName("BenchMarkTimestampMultipleReceivers")
    ...
    val rawStreams = (1 to numStreams).map( index => 
            ssc.socketTextStream(host, port + index - 1,
            StorageLevel.MEMORY_ONLY_SER)).toArray
    val times = ssc.union(rawStreams);
